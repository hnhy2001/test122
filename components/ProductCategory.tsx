...
const ProductCategory = ({category, setCategory, className}: any) => {
  ...
  const fetchData = () => {
    getRequest(
      `/product/list-category-level-3?keyword=${input}&page=${page}&limit=5`
    )
      .then((data) => {
        setPage((prev) => prev + 1);
  ...
  return (
    <Popover open={openCombobox} onOpenChange={setOpenCombobox} modal={true}>
      <PopoverTrigger asChild>
        ...
        >
          {category ? (
            ...
            <div className="text-[#acafb3] font-normal text-xl">
              Select category...
            </div>
          )}
      ...
      <PopoverContent
        className={cn(
          "w-[calc(100vw-6rem)] xs:w-[calc(60vw-6rem)] p-0",
          className
        )}
      >
        <div className="w-full p-4">
          <div className="pb-4">
            <Input
              placeholder="Type keyword then hit enter to search"
              onChange={(e) => {
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  ...
                  setLoadingSearch(true);
                  getRequest(
                    `/product/list-category-level-3?keyword=${input}&page=1&limit=5`
                  ).then((data: any) => {
                    setCategoryies(data.data);
                    setLoadingSearch(false);
                  });
          ...
          <div
            id="myContainer"
            className="overflow-auto h-[18rem]"
            ref={containerRef}
          >
            {!loadingSearch && categories.length == 0 && (
